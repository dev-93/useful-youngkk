...
마이홈 포털(www.myhome.go.kr) 공공주택 청약 정보를 크롤링하여 구조화된 데이터로 저장한다.
"""
...
# 마이홈 포털 기본 URL
MYHOME_BASE_URL = "https://www.myhome.go.kr"
MYHOME_LIST_URL = f"{MYHOME_BASE_URL}/hws/portal/sch/selectRsdtRcritNtcList.do"
MYHOME_DETAIL_URL = f"{MYHOME_BASE_URL}/hws/portal/sch/selectRsdtRcritNtcDetailView.do"
...
class MyHomeCrawler(BaseCrawler):
    ...
    def fetch_list(self) -> str:
        """마이홈 포털 공고 목록 페이지 HTML을 가져온다."""
        logger.info("[MyHome] 목록 페이지 요청: %s", MYHOME_LIST_URL)
        response = self._client.get(MYHOME_LIST_URL)
    ...
    def parse_list(self, html: str) -> list[ListItem]:
        ...
        마이홈 포털은 테이블 또는 리스트 형태로 공고 목록을 제공한다.
        각 항목에서 제목, 링크(source_id 포함)를 추출한다.
        """
        ...
        if not rows:
            rows = soup.select(".board_list .list_item, .bbs_list tbody tr, .lst_type li")
        ...
        for row in rows:
            try:
                ...
                logger.debug("[MyHome] 목록 행 파싱 실패: %s", str(e))
                continue
    ...
    def _extract_source_id(self, href: str) -> str | None:
        ...
        마이홈 포털 URL 파라미터: pblancId, ntcSn, sn, rcritId 등
        """
        patterns = [
            r"[?&]pblancId=([A-Za-z0-9]+)",
            r"[?&]ntcSn=(\d+)",
            r"[?&]sn=(\d+)",
            r"[?&]rcritId=([A-Za-z0-9]+)",
            r"[?&]seq=(\d+)",
        ...
        js_match = re.search(
            r"(?:fn_view|goView|detail|goDetail)\s*\(\s*['\"]?([A-Za-z0-9]+)", href
        )
        if js_match:
    ...
    def _build_detail_url(self, href: str, source_id: str) -> str:
        ...
        if href.startswith("http"):
            return href
        if href.startswith("/"):
            return f"{MYHOME_BASE_URL}{href}"
        return f"{MYHOME_DETAIL_URL}?pblancId={source_id}"
    ...
    def fetch_detail(self, item: ListItem) -> str:
        ...
        logger.info("[MyHome] 상세 페이지 요청: %s", item.detail_url)
