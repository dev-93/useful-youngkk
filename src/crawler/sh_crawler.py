...
SH공사 청약 공고 목록 및 상세 페이지를 크롤링하여 구조화된 데이터로 저장한다.
"""
...
SH_LIST_URL = f"{SH_BASE_URL}/main/lay2/program/S1T295C297/www/brd/m_247/list.do"
SH_DETAIL_URL = f"{SH_BASE_URL}/main/lay2/program/S1T295C297/www/brd/m_247/view.do"
...
class SHCrawler(BaseCrawler):
    ...
    def fetch_list(self) -> str:
        """SH공사 공고 목록 페이지 HTML을 가져온다."""
        logger.info("[SH] 목록 페이지 요청: %s", SH_LIST_URL)
        response = self._client.get(SH_LIST_URL)
    ...
    def parse_list(self, html: str) -> list[ListItem]:
        ...
        for row in rows:
            try:
                ...
                logger.debug("[SH] 목록 행 파싱 실패: %s", str(e))
                continue
    ...
    def _build_detail_url(self, href: str, source_id: str) -> str:
        ...
        if href.startswith("http"):
            return href
        if href.startswith("/"):
            return f"{SH_BASE_URL}{href}"
        # 기본 상세 URL 구성
        return f"{SH_DETAIL_URL}?seq={source_id}"
    ...
    def fetch_detail(self, item: ListItem) -> str:
        ...
        logger.info("[SH] 상세 페이지 요청: %s", item.detail_url)
    ...
    def _extract_period(self, soup, body_text: str) -> tuple[date | None, date | None]:
        ...
        # 기간 표시 패턴: YYYY.MM.DD ~ YYYY.MM.DD
        period_patterns = [
